      ...
      filterSubmitHandle: async function (form) {
        ...
        if (form.chartSelected.length > 0) {
          ...
          if (vm.showDailyChart) {
            ...
            await ChartsApi.fetchstatamazonlinedaily('/v1/statamazonlinetotalitemsday/', {
              order: ['dy','line_id'],
              size: 500000
          ...
          if (vm.showMonthlyChart) {
            ...
            await ChartsApi.fetchstatamazonlinemonthly('/v1/statamazonlinetotalitemsmonth/', {
              order: ['yr','mo','line_id'],
              size: 500000
            }, queryParams).then(response => {
              vm.monthlyChartOptions = ChartDataHandler.lineMonthlyChangeHandle(vm.monthlyChart, response.data, vm);
            });
          ...
          if (vm.showWeeklyChart) {
            ...
            queryParams.push([{'first_day-gte-and': _beginDate}]);
            queryParams.push([{'last_day-lte-and': _endDate}]);
            await ChartsApi.fetchstatamazonlineweekly('/v1/statamazonlinetotalitemsweek/', {
              order: ['yr','wk','line_id'],
              size: 500000
            }, queryParams).then(response => {
              vm.weeklyChartOptions = ChartDataHandler.lineWeeklyChangeHandle(vm.weeklyChart, response.data, vm);
            });
          }
        }
      },
