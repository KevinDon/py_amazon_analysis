...
<script>
  ...
  export default {
    name: "chartLineUserConversionRateChart",
    components: {
    ...
    mounted: function () {
      ...
      // vm.weeklyChart = echarts.init(document.getElEementById('chartWeekly'), 'canvas', 'auto', 'auto');
      window.onresize = () => {
        return (() => {
      ...
      filterSubmitHandle: async function (form) {
        ...
        if (form.chartSelected.length > 0) {
          ...
          if (vm.showDailyChart) {
            ...
            await ChartsApi.fetchstatamazonlinedaily('/v1/statamazonlineuvitemsconversionrateday/', {
              order: ['dy','line_id'],
              size: 500000
