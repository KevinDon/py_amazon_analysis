...
<script>
  ...
  export default {
    name: "chartSkuCompositeReportChart",
    components: {
      ...
      filterSubmitHandle: async function (form) {
        ...
        if (form.chartSelected.length > 0) {
          ...
          if (vm.showDailyChart) {
            ...
            await ChartsApi.statamazonskutotalitemsday('/v1/statamazonskucompositereportday/',{}, queryParams).then(response => {
              vm.daliyChartOptions = ChartDataHandler.skuDayCompositeHandle(vm.dailyChart, response.data, vm);
            });
          }
          ...
          if (vm.showMonthlyChart) {
            ...
            await ChartsApi.statamazonskutotalitemsmonth('/v1/statamazonskucompositereportmonth/',{}, queryParams).then(response => {
              vm.monthlyChartOptions = ChartDataHandler.skuMonthCompositeHandle(vm.monthlyChart, response.data, vm);
            });
          ...
          if (vm.showWeeklyChart) {
            ...
            queryParams.push([{'first_day-gte-and': _beginDate}]);
            queryParams.push([{'last_day-lte-and': _endDate}]);
            await ChartsApi.statamazonskutotalitemsweek('/v1/statamazonskucompositereportweek/',{}, queryParams).then(response => {
              vm.weeklyChartOptions = ChartDataHandler.skuWeeklyCompositeHandle(vm.weeklyChart, response.data, vm);
            });
          }
        }
      },
