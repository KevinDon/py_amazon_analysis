...
import_parent_file = 'batch_parent_sku_list.csv'

# parent sku - not ok
with open(os.path.relpath(os.path.join(os.path.dirname(__file__),import_parent_file))) as f:
    csv_reader = csv.reader(f)
    columns = next(csv_reader)
    for row in csv_reader:
        if row[0] is '':
            continue
        # if row[5] is not '' and int(row[5]) == 0:
        #     continue
        try:
            _single_product = update_product_asin(sku=row[0], asin=row[1] or None, combine_type=5)
