...
import_single_file = 'batch_combined_sku_department.csv'
...
with open(os.path.realpath(os.path.join(os.path.dirname(__file__), import_single_file))) as f:
    ...
    for row in csv_reader:
        if row[0] is '':
            continue
        try:
            _single_product = ProductAsinModel.objects.filter(Q(sku=row[0])).exclude(Q(combine_type=1)).all()
            _department = DepartmentModel.objects.filter(Q(code=row[1])).first()

            if _single_product is None:
                raise ('缺少对应的产品,sku为 %s ' % row[0])

            if _department is None:
                raise ('缺少对应的部门,id为 %s ' % row[1])

            for item in _single_product:
