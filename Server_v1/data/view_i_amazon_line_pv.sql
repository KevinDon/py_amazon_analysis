-- amazon analysis page view stat
...
-- DROP MATERIALIZED VIEW IF EXISTS view_i_amazon_line_pv_day;
CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."view_i_amazon_line_pv_day" AS
    SELECT row_number() OVER () AS id,
       ...
       FROM (( SELECT line.title,
         ...
         LEFT JOIN ( SELECT repo.line_id,
                repo.dy,
                sum(repo.page_view) AS num
               FROM ( SELECT rp.page_view,
                        pro.line_id,
...
-- DROP MATERIALIZED VIEW IF EXISTS view_i_amazon_line_pv_month;
CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."view_i_amazon_line_pv_month" AS
    SELECT row_number() OVER () AS id,
        main.title,
        main.line_id,
        main.yr,
...
-- DROP MATERIALIZED VIEW IF EXISTS view_i_amazon_line_pv_week;
CREATE MATERIALIZED VIEW IF NOT EXISTS "public"."view_i_amazon_line_pv_week" AS
    SELECT row_number() OVER () AS id,
        main.title,
        main.line_id,
        main.yr,
